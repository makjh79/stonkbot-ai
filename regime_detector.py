...
def _fetch_fresh_regime_data() -> Dict:
    ...
    try:
        ...
        # Fetch 20-day historical bars for direction comparison
        try:
            hist = hub.get_daily_bars(["SHY", "TLT", "LQD", "HYG"], days=25)
            shy_hist = hist.get("SHY", {}).get("closes", [])
            tlt_hist = hist.get("TLT", {}).get("closes", [])
            lqd_hist = hist.get("LQD", {}).get("closes", [])
            hyg_hist = hist.get("HYG", {}).get("closes", [])
        except Exception:
            shy_hist = tlt_hist = lqd_hist = hyg_hist = []

        # Yield curve proxy: SHY/TLT ratio (compare to 20-day baseline)
        shy = regime.get("SHY", {}).get("price", 0)
        tlt = regime.get("TLT", {}).get("price", 0)
        if tlt > 0:
            regime["yield_curve_ratio"] = round(shy / tlt, 4)
            if len(shy_hist) >= 20 and len(tlt_hist) >= 20:
                ratio_prev = shy_hist[-20] / tlt_hist[-20]
                regime["yield_curve_signal"] = "steepening" if (shy / tlt) > ratio_prev else "normal"
            else:
                regime["yield_curve_signal"] = "normal"  # fallback

        # Credit spread proxy: LQD/HYG ratio (compare to 20-day baseline)
        lqd = regime.get("LQD", {}).get("price", 0)
        hyg = regime.get("HYG", {}).get("price", 0)
        if hyg > 0:
            ratio = lqd / hyg
            regime["credit_spread_ratio"] = round(ratio, 4)
            if len(lqd_hist) >= 20 and len(hyg_hist) >= 20:
                ratio_prev = lqd_hist[-20] / hyg_hist[-20]
                regime["credit_signal"] = "widening" if ratio > ratio_prev else "improving"
            else:
                regime["credit_signal"] = "improving"  # fallback
