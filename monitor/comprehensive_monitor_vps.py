...
def check_factor_confirmation_integrity() -> None:
    ...
    expected_bool_keys = [
        ...
        "options_confirmed", "earnings_confirmed",
    ]
    ...
    for sig in signals.get("signals", []):
        ...
        if strategy == "mean_reversion":
            ...
            expected_keys = {
                ...
                "options_confirmed", "options_score", "earnings_confirmed",
            }
            max_count = 9
...
def check_dead_code() -> None:
    """Dead-factor lint in /opt/stonk-ai is the single source of truth."""
    return
        ...
        for keyword, field, is_confirmed in FACTORS:
            ...
            for pat in MISSING_PATTERNS:
                ...
                if re.search(regex, text):
                    _log_issue(f"SEMANTIC CONTRADICTION {sym} ({src}): text implies '{keyword}' is missing but {field}={val} (confirmed)")
