...
class BacktestEngine:
    ...
    def __init__(
        ...
        benchmark: str = "SPY",
        use_old_weights: bool = False,
    ):
        self.start_date = start_date
        self.end_date = end_date
        self.initial_cash = initial_cash
        self.cash = initial_cash
        self.positions: Dict[str, Dict] = {}  # symbol -> {qty, cost_basis, entry_date, entry_price}
        ...
        self.drawdown_halt: bool = False  # Halt new entries when DD < -10%
    ...
    def generate_signal_for_symbol(self, symbol: str, bars: Dict, all_bars: Dict) -> Optional[Dict]:
        ...
        # Override weights if running old-weight baseline
        if self.use_old_weights:
            import readiness_score as rs
            original_weights = {
                'WEIGHT_EMA': rs.WEIGHT_EMA,
                'WEIGHT_RELATIVE_STRENGTH': rs.WEIGHT_RELATIVE_STRENGTH,
            }
            rs.WEIGHT_EMA = 0.12
            rs.WEIGHT_RELATIVE_STRENGTH = 0.0
            try:
                ...
                rs.WEIGHT_EMA = original_weights['WEIGHT_EMA']
                rs.WEIGHT_RELATIVE_STRENGTH = original_weights['WEIGHT_RELATIVE_STRENGTH']
        else:
            readiness = compute_readiness(
                symbol=symbol,
                total_score=total,
        ...
        for i, date_str in enumerate(dates):
            ...
            if pending_orders:
                for order in pending_orders:
                    ...
                    if order["action"] == "BUY":
                        # Live bot rules: only PRIME/STRONG_NOW entries, 12% max position
                        qty = int(order["target_value"] / exec_price) if exec_price > 0 else 0
                        if qty > 0:
                            # Apply slippage to cost (not price) so it actually impacts returns
                            cost = qty * exec_price * (1 + self.slippage_bps / 10000)
                            self.cash -= cost
                            self.positions[sym] = {"qty": qty, "cost_basis": cost, "entry_date_idx": 0, "entry_price": cost / qty, "entry_date": date_str}
                            self.trades.append({
                                "date": date_str,
                                "action": "BUY",
                                "symbol": sym,
            ...
            # Check stop losses first (-5% hard cut, ATR trailing stop, VWAP floor)
            for sym in list(self.positions.keys()):
                pos = self.positions.get(sym, {})
                entry_price = pos.get("entry_price", 0)
                if entry_price <= 0 or sym not in current_prices:
                    continue
                ...
                # Hard -5% cut
                if loss_pct <= -0.05:
                    pending_orders.append({"action": "SELL", "symbol": sym, "reason": "hard_cut_-5pct"})
                    continue

                # ATR trailing stop (1.5x hard, 2.0x trailing, clamped to 3-8%)
                atr = pos.get("atr14", 0)
                if atr > 0 and entry_price > 0:
                    atr_pct = atr / entry_price
                    stop_pct = min(max(1.5 * atr_pct, 0.03), 0.08)
                    if loss_pct <= -stop_pct:
                        pending_orders.append({"action": "SELL", "symbol": sym, "reason": "atr_stop"})
                        continue

                # Thesis broken: readiness < 40
                sym_signal = next((s for s in signals if s["symbol"] == sym), None)
                if sym_signal and sym_signal["readiness_score"] < 40:
                    pending_orders.append({"action": "SELL", "symbol": sym, "reason": "thesis_broken"})
                    continue

            # Queue sells for tomorrow at T+1 open (tier demotion / flat exit)
            for sym in list(self.positions.keys()):
                ...
                # Flat exit after 5 days if not still entry-eligible PRIME
                sym_signal = next((s for s in signals if s["symbol"] == sym), None)
                if days_held >= 5 and (not sym_signal or not sym_signal.get("entry_eligible")):
                    pending_orders.append({"action": "SELL", "symbol": sym, "reason": "flat_exit_5d"})
                    continue

                # If not in top PRIME signals, queue sell
                if sym not in top_symbols:
                    if sym_signal and sym_signal["readiness_score"] >= 55:
                        pos["entry_date_idx"] = days_held + 1
                        continue
                    # Queue sell for tomorrow
                    pending_orders.append({"action": "SELL", "symbol": sym, "reason": "tier_demotion"})
            ...
            target_signals = [s for s in top_signals if s["symbol"] not in self.positions and s.get("entry_eligible") and s.get("tier") == "STRONG_NOW"]
            if target_signals and deployable_cash > 0 and not self.drawdown_halt:
                # Live bot: 12% max per STRONG_NOW position
                for sig in target_signals[:12]:
                    sym = sig["symbol"]
                    if sym in self.positions or sym not in current_prices:
                        continue
                    target_value = min(0.12 * self._portfolio_value(current_prices), deployable_cash)
...
def main():
    ...
    parser.add_argument("--old-weights", action="store_true", help="Use pre-relative-strength weights for baseline")
    ...
    engine = BacktestEngine(
        ...
        use_old_weights=args.old_weights,
    )
