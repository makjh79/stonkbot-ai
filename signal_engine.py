...
from readiness_score import compute_readiness, ReadinessResult
...
class SignalEngine:
    ...
    def save_signals(self, signals: List[Signal], path: Optional[Path] = None) -> Path:
        ...
        all_signals = momentum_dicts + mr_signals
        logger.info(f"Signal merge: {len(momentum_dicts)} momentum + {len(mr_signals)} mean reversion = {len(all_signals)} total")
        ...
        logger.info(f"Saved {len(all_signals)} signals to {path}")
        return path
    ...
    def _score_symbol(
        ...
        return Signal(
            ...
            options_implied_vol=round(e.get("options", {}).get("avg_implied_vol", 0) or 0, 3) if e.get("options", {}).get("avg_implied_vol") else None,
    ...
    def _fetch_options_enrichment(self, symbols: List[str]) -> Dict[str, Dict]:
        """Fetch options sentiment data (implied vol, volume) from Alpaca."""
        result = {}
        try:
            for symbol in symbols:  # Fetch for ALL symbols (was limited to top 30)
                snap = self._hub.get_options_snapshot(symbol)
                if snap:
                    result[symbol] = {
                        "options_volume": snap.get("total_options_volume", 0),
                        "avg_implied_vol": snap.get("avg_implied_vol"),
                        "contract_count": snap.get("contract_count", 0),
                    }
        except Exception as e:
...
if __name__ == "__main__":
    ...
    for s in signals[:10]:
        print(
            ...
            f"score={s.total_score:.1f} conf={s.confirmation_count}/5 "
