...
class RiskConfig:
    ...
    min_cash_pct: float = 0.05                  # hold at least 5% cash for dip buying and safety
    entry_cash_buffer_pct: float = 0.07           # leave 7% cash after new buys (buffer above 5% floor)
    slippage_buffer: float = 0.98               # assume 2% worse fill

    # --- Dynamic cash floor (scales with drawdown) ---
    # At high water mark: min_cash_pct (5%). As drawdown deepens, floor rises.
    ...
    def __post_init__(self):
        ...
        if self.dynamic_cash_floor is None:
            self.dynamic_cash_floor = {
                0.00: 0.05,   # at high: 5% floor
                0.05: 0.06,   # -5% drawdown: 6% floor
                0.10: 0.08,   # -10% drawdown: 8% floor
                0.15: 0.10,   # -15% drawdown: 10% floor
                0.20: 0.12,   # -20%+ drawdown: 12% floor
            }

    # --- Concentration ---
    max_single_position_pct: float = 0.08          # tightened from 10% — backtest showed -40.6% DD
    max_sector_pct: float = 0.20                   # no sector > 20%
    concentration_trim_trigger: float = 1.25      # only trim if 125% over cap

    # --- Loss / drawdown controls ---
    new_entry_max_drawdown_pct: float = -0.15     # halt new buys at -15% DD (was -30%, too lenient)
    ...
    trailing_stop_atr_multiplier: float = 2.0       # tightened from 2.5 — reduce drawdown
    ...
    # --- Universe / gate ---
    top_signal_count: int = 15                      # consider top 15 signals
...
class RiskEngine:
    ...
    def check_exits(self, portfolio_data: Dict) -> List[Dict]:
        ...
        for pos in portfolio_data.get("positions", []):
            ...
            if plpc <= self.config.hard_stop_loss_pct:
                trades.append({
                    "symbol": symbol,
                    "qty": qty,
                    "action": "SELL",
                    "reason": f"Hard stop: {plpc:.1%} (limit {self.config.hard_stop_loss_pct:.1%})",
                })
                logger.warning(f"STOP LOSS: {symbol} at {plpc:.1%}")
                continue
            ...
            # 2. Trailing stop from position high water mark (volatility-aware)
            atr_pct = self.position_atr_pct.get(symbol)
            if atr_pct and atr_pct > 0:
                atr_trailing = -(atr_pct * self.config.trailing_stop_atr_multiplier)
                effective_trailing = min(self.config.trailing_stop_pct, atr_trailing)
            else:
                effective_trailing = self.config.trailing_stop_pct
