...
class StrategyConfig:
    ...
    MIN_CASH_BUFFER = 0.30  # Keep only 30% cash (deploy 70% capital)
    
    # Concentrated Core Holdings (The Council's Tier 1)
    CORE_POSITIONS = ['PLTR', 'AMD', 'CRWD', 'HOOD', 'AAPL', 'NVDA']
    TIER_2_POSITIONS = ['GOOGL', 'AVGO']  # Hold but don't add
    SELL_LIST = ['MSFT', 'SOFI', 'SCHD', 'SGOV']  # Paulson + Jones say sell
    ...
    ALLOWED_RSI_SYMBOLS = ['PLTR', 'AMD', 'CRWD', 'HOOD', 'NVDA', 'GOOGL']  # Top 6
...
class STONKAIBot:
    ...
    def check_rsi_entries(self, portfolio_data: Dict) -> List[Dict]:
        ...
        # Get watchlist stocks to monitor
        watchlist_symbols = ['DKNG', 'COIN', 'UPST', 'ROKU', 'ABNB', 'SHOP', 'SQ', 'RIVN', 'NET', 'SNOW']
        ...
        # Only enter if we have enough cash
        if cash < portfolio_value * 0.10:  # Need at least 10% cash
            return entries
            ...
            try:
                ...
                if rsi and rsi <= StrategyConfig.RSI_ENTRY_THRESHOLD:
                    ...
                    if volume_ok:
                        ...
                        position_value = portfolio_value * StrategyConfig.RSI_ENTRY_POSITION_SIZE
                        qty = max(1, int(position_value / self.get_current_price(symbol)))
                        
                        entries.append({
                            'symbol': symbol,
                            'qty': qty,
                            'action': 'BUY',
                            'reason': f'RSI Auto-Entry: RSI {rsi:.1f} (below {StrategyConfig.RSI_ENTRY_THRESHOLD}) with volume confirmation',
                            'rsi': rsi
                        })
                        logger.info(f"RSI ENTRY SIGNAL: {symbol} at RSI {rsi:.1f}")
            except Exception as e:
