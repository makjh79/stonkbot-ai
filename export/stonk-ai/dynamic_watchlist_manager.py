...
# Configuration - MORE AGGRESSIVE THRESHOLDS
WATCHLIST_CONFIG = {
    'max_stocks': 20,
    'min_stocks': 8,
    'refresh_interval_minutes': 5,
    # Thresholds for removal - MORE AGGRESSIVE
    'remove_rsi_above': 65,  # Was 75 - earlier profit taking
    'remove_rsi_below': 20,  # Lowered to avoid conflict with bot entry (RSI <35)
    'remove_price_change_day': 8,  # Was 15 - rotate on any big day
    'remove_underperform_sp500': -5,  # NEW: Cut if lagging S&P by 5%
    # Thresholds for addition - widened to catch oversold opportunities
    'add_rsi_min': 25,  # Widen to catch more oversold setups (bot buys at <35)
    'add_rsi_max': 60,
    'add_ai_score_min': 55,  # Lowered from 60
}
...
def evaluate_stock(symbol, data, sp500_return):
    """Evaluate if stock should stay in watchlist - MORE AGGRESSIVE"""
    if not data:
        return {'keep': False, 'reason': 'No data available', 'action': 'REPLACE'}
    
    # Skip permanent positions (hedges)
    if symbol in PERMANENT_POSITIONS:
        return {'keep': True, 'reason': 'Hedge position - permanent', 'action': 'KEEP'}
    
    rsi = data.get('rsi') or 50
    change_pct = data.get('change_pct') or 0
    
    # Check removal conditions - NOW MORE AGGRESSIVE
    if rsi > WATCHLIST_CONFIG['remove_rsi_above']:
        return {'keep': False, 'reason': f'RSI {rsi:.1f} overbought (>{WATCHLIST_CONFIG["remove_rsi_above"]})', 'action': 'REPLACE'}
    
    if rsi < WATCHLIST_CONFIG['remove_rsi_below']:
        return {'keep': False, 'reason': f'RSI {rsi:.1f} oversold (<{WATCHLIST_CONFIG["remove_rsi_below"]})', 'action': 'REPLACE'}
    
    if change_pct > WATCHLIST_CONFIG['remove_price_change_day']:
        return {'keep': False, 'reason': f'Up {change_pct:.1f}% today (take profits)', 'action': 'REPLACE'}
    
    # NEW: Check vs S&P 500 underperformance
    vs_sp500 = change_pct - sp500_return
    if vs_sp500 < WATCHLIST_CONFIG['remove_underperform_sp500']:
        return {'keep': False, 'reason': f'Underperforming S&P by {abs(vs_sp500):.1f}%', 'action': 'REPLACE'}
    
    return {'keep': True, 'reason': f'RSI {rsi:.1f}, within thresholds', 'action': 'KEEP'}
...
def update_watchlist():
    ...
    print(f"📊 Config: RSI >{WATCHLIST_CONFIG['remove_rsi_above']} | <{WATCHLIST_CONFIG['remove_rsi_below']} | Daily >{WATCHLIST_CONFIG['remove_price_change_day']}% | vs S&P <{WATCHLIST_CONFIG['remove_underperform_sp500']}%")
    ...
    if needed > 0:
        ...
        for candidate in candidates[:needed]:
            ...
            print(f"  ➕ Adding {candidate['symbol']}: {candidate['reason']}")
