...
# Sector peer map for breadth features
SECTOR_PEERS = {
...
for sym, data in bars.items():
    ...
    for i in range(50, len(c) - 5):
        ...
        # Sector breadth: % of sector peers above 20d EMA
        peers_above_ema = 0
        peers_total = 0
...
print(f'Total 2yr samples: {len(records)}')
with open('/opt/stonk-ai/v3_rebuild/data/features_2yr.json', 'w') as f:
    json.dump(records, f)
print('Saved features_2yr.json')
