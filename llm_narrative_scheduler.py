...
STATUS_FILE = Path(os.environ.get("STONKBOT_LLM_STATUS", "/opt/stonk-ai/.llm_narrative_status"))
...
def main() -> int:
    try:
        ...
        try:
            STATUS_FILE.write_text(json.dumps({"mode": reason, "ts": datetime.now(timezone.utc).isoformat()}))
        except Exception:
            pass
        if not run:
            return 0
