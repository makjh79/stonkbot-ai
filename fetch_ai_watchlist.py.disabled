            ...
            for symbol in watchlist_symbols:
                ...
                if symbol in quotes_data:
                    ...
                    prices[symbol] = {
                        ...
                        'signal_tier': {'STRONG_NOW': 'PRIME', 'NOW': 'BUILDING', 'WATCH': 'WATCHING', 'MONITOR': 'TRACKING'}.get(sig_data.get('tier', 'MONITOR'), 'TRACKING'),
                        'readiness_score': sig_data.get('readiness_score', 0),
                        'entry_eligible': sig_data.get('entry_eligible', False),
                        'tier_reason': sig_data.get('tier_reason', ''),
                        'confirmation_count': compute_confirmation_count(sig_data) if sig_data else 0,
...
def save_watchlist_data(prices):
    ...
    try:
        atomic_write_json(opt_file, data)
    ...
    try:
        ...
        try:
            ...
            pass
        # ---------------------------
        atomic_write_json(web_file, data_to_save)
...
def _enrich_with_tiers(prices):
    """Backwards-compatible noop: tiers are owned by dynamic_watchlist_manager.py.
    Price updates must preserve whatever tier/confirmations metadata already exists."""
    pass


def _enrich_with_confirmations(prices):
    """Copy confirmations metadata from signals.json for popup rendering."""
    try:
        import json
        from pathlib import Path
        ...
        for sym, data in prices.items():
            ...
            # Always recompute count from the confirmations dict
            if isinstance(data.get("confirmations"), dict):
                data["confirmation_count"] = sum(
                    1 for v in data["confirmations"].values() if v is True
                )
    except Exception as e:
        logger.debug(f"Failed to enrich confirmations: {e}")
...
def main():
    ...
    while True:
        ...
        if prices:
            ...
            _enrich_with_confirmations(prices)
