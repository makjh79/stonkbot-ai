...
class SignalTracker:
    ...
    def detect_new_signals(self):
        ...
        for symbol, data in watchlist.get('prices', {}).items():
            # Check if this is an entry signal (NOW or STRONG_NOW tier)
            rsi = data.get('rsi', 50)
            ...
            # Detect entry signals: STRONG_NOW or NOW tiers, or legacy BUY
            is_entry = tier in ('STRONG_NOW', 'NOW') or signal == 'BUY'
            ...
            if is_entry:
                ...
                if not existing:
                    ...
                    print(f"🎯 New entry signal: {symbol} @ ${data.get('price', 0):.2f} (tier: {tier}, RSI: {rsi:.1f})")
    ...
    def calculate_accuracy_stats(self):
        ...
        # Only consider signals with tracker fields (skip engine-only signals)
        tracked = [s for s in self.signals['signals'] if 'status' in s]
        closed_signals = [s for s in tracked if s['status'] == 'closed']
        ...
        if not closed_signals:
            return {
                ...
                'pending_signals': len([s for s in self.signals['signals'] if s.get('status') == 'active'])
            }
    ...
    def export_for_website(self):
        ...
        recent = sorted(
            [s for s in self.signals['signals'] if s.get('status') == 'active'],
...
# === Compatibility wrappers for update_signal_performance.py ===

def load_signal_history():
    """Compatibility wrapper - returns list of tracked signals."""
    tracker = SignalTracker()
    signals = tracker.signals.get('signals', [])
    # Only return signals that have tracker fields (status/outcome)
    return [s for s in signals if 'status' in s or 'outcome' in s]

def save_signal_history(signals):
    """Compatibility wrapper - saves signal list back to file."""
    tracker = SignalTracker()
    # Merge: keep engine signals + replace tracked ones
    engine_signals = [s for s in tracker.signals.get('signals', []) if 'status' not in s and 'outcome' not in s]
    tracker.signals['signals'] = engine_signals + signals
    tracker.save_signals()

def get_signal_stats(days=30):
    """Compatibility wrapper - returns accuracy stats for last N days."""
    tracker = SignalTracker()
    stats = tracker.calculate_accuracy_stats()
    stats.setdefault('total_signals', stats.get('total_signals', 0))
    stats.setdefault('completed_signals', stats.get('closed_signals', 0))
    stats.setdefault('pending_signals', stats.get('pending_signals', 0))
    stats.setdefault('win_rate', stats.get('win_rate', 0))
    stats.setdefault('avg_return', stats.get('avg_return', 0))
    return stats

def export_for_website():
    """Compatibility wrapper - exports accuracy data for website."""
    tracker = SignalTracker()
    return tracker.export_for_website()
