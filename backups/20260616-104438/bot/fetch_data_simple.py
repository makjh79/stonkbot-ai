...
def update_history(data):
    ...
    try:
        ...
        # Keep strategy: last 500 entries for detail, but ensure we keep one per day for last 30 days
        checks = history["checks"]
        if len(checks) > 500:
            # Get dates from checks
            dates_kept = set()
            filtered = []
            
            # Go through in reverse (newest first)
            for c in reversed(checks):
                date = c["timestamp"][:10]  # YYYY-MM-DD
                if date not in dates_kept or len(filtered) < 500:
                    filtered.insert(0, c)
                    dates_kept.add(date)
            
            history["checks"] = filtered[-500:]  # Keep last 500
