...
def _why_on_watchlist(signal_data, watchlist_data):
    ...
    if entry_eligible:
        if tier == "STRONG_NOW":
            return f"Entry-ready at STRONG_NOW tier (readiness {readiness:.0f}, {conf_count}/10 confirmations). Highest conviction — bot will buy with 1.5x sizing."
        else:
            return f"Entry-ready at NOW tier (readiness {readiness:.0f}, {conf_count}/10 confirmations). Bot will buy when cash is available."
    elif tier == "WATCH":
        gap = 72 - readiness
        if gap > 0:
            return f"WATCH tier (readiness {readiness:.0f}). Needs {gap:.0f} more points to reach entry threshold (72). {conf_count}/10 confirmations — looking for more signal strength."
        else:
            return f"WATCH tier (readiness {readiness:.0f}). Close to entry but missing confirmations ({conf_count}/10, need 2+)."
    elif tier == "MONITOR":
...
def _what_triggers_buy(signal_data, watchlist_data):
    ...
    missing = []
    if readiness < 72:
        missing.append(f"readiness needs to reach 72 (currently {readiness:.0f})")
    if conf_count < 2:
        missing.append(f"needs 2+ confirmations (currently {conf_count})")
