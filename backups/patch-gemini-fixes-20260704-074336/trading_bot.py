...
- Readiness-driven: entry_eligible (readiness >= 75 AND >= 4 confirmations AND above_ema) replaces hard score threshold.
...
v2.1 changes:
  ...
  - Position sizing scaled by readiness: >=78=1.5x (STRONG_NOW), >=72=1.0x (NOW), <72=0.65x
...
class STONKAIBot:
    ...
    def _is_entry_eligible_for_mode(self, sig: dict) -> bool:
        """Entry gate: looser for paper, strict for live."""
        ...
        above_ema = sig.get("above_ema20", False) or sig.get("confirmations", {}).get("above_ema", False)
        return readiness >= 70.0 and conf >= 3 and above_ema
    ...
    def _readiness_sizing_multiplier(readiness: float) -> float:
        ...
        Aggressive dispersion: top signals get 3x, mid gets 1.5x, weak gets 0x."""
        if readiness >= 80:
            return 3.0   # STRONG_NOW high conviction
        if readiness >= 75:
            return 1.5   # upper NOW
        if readiness >= 72:
    ...
    def run_cycle(self):
        ...
        if self._regime == "CRISIS":
            ...
            _min_hold_days = 5   # alpha: cut losers fast, recycle into STRONG_NOW
        ...
        # If held >= 10 days AND price within ±3% of entry AND readiness < 75, exit
        if self._regime != "CRISIS":  # CRISIS already handles fast exits
            ...
            if _entry > 0 and _current > 0:
                ...
                if _loss_pct <= -5.0:
                    ...
                    pass  # continue removed: outside loop
...
# ALPHA: Hard portfolio position ceiling -- trim weakest when over limit, block new entries at limit
    ...
    def run(self):
        ...
        if self.alpaca.is_paper():
            logger.info(f"Entry: PAPER readiness >= 70 AND >= 3 confirmations AND above_ema")
        else:
            logger.info(f"Entry: LIVE readiness >= 75 AND >= 4 confirmations AND above_ema")
