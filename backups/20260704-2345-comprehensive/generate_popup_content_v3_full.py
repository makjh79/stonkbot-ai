...
def generate_dynamic_narrative(symbol, position, watchlist_data, signal_data, risk_config, risk_state):
    ...
    confirmation_count = compute_confirmation_count(signal_data.get("confirmations", {}))
...
def _why_on_watchlist(signal_data, watchlist_data):
    ...
    if entry_eligible:
        if tier == "PRIME":
            return f"Entry-ready at PRIME tier (readiness {readiness:.0f}, {conf_count}/10 confirmations). Highest conviction — bot will buy with 2.0x sizing."
        else:
            # Should not happen under Option B (only PRIME is tradeable)
        ...
        if gap_conf > 0:
            return f"PRIME tier (readiness {readiness:.0f}) but missing {gap_conf} confirmation(s) to reach entry gate ({conf_count}/10, need 5+)."
        elif not watchlist_data.get("above_ema", signal_data.get("above_ema20", False)):
            return f"PRIME tier (readiness {readiness:.0f}, {conf_count}/10 confirmations) but price is not above EMA."
        else:
            return f"PRIME tier (readiness {readiness:.0f}, {conf_count}/10 confirmations). Entry conditions met if capital is available."
    elif tier == "BUILDING":
        gap = 78 - readiness
        if gap > 0:
...
def _what_triggers_buy(signal_data, watchlist_data):
    ...
    if tier != "PRIME":
        gap = 78 - readiness
        if gap > 0:
            missing.append(f"reach PRIME tier at readiness 78 (currently {readiness:.0f})")
        else:
            missing.append("cross into PRIME tier (readiness already near 78)")
    if conf_count < 5:
    ...
    if not missing:
        return "PRIME conditions met — entry will trigger when capital is available."
