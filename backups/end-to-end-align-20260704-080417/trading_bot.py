...
class STONKAIBot:
    ...
    def _is_entry_eligible_for_mode(self, sig: dict) -> bool:
        """Entry gate: use signal's entry_eligible flag; fall back only in paper mode to a tightened gate.
        Tightened 2026-07-04 while live expectancy is negative.
        """
        if sig.get("entry_eligible", False):
            return True
        if not getattr(self.alpaca, "is_paper", lambda: False)():
            return False
        # Paper fallback only: match tightened readiness_score.py gate
        ...
        return readiness >= 77.0 and conf >= 5 and above_ema
