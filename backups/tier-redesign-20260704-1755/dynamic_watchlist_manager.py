...
- Assign tiers based on readiness:
    STRONG_NOW / PRIME: readiness >= 78 (only tradeable tier, 2.0x sizing, 12% cap)
    NOW / BUILDING: readiness 72-77 (non-trading, building strength)
    WATCH / WATCHING: readiness 55-71
    MONITOR / TRACKING: readiness < 55
...
from readiness_score import ENTRY_MIN_CONFIRMATIONS
...
def build_watchlist(signals: List[Dict]) -> Dict:
    ...
    # User-friendly display tier labels. Tiers reflect signal strength (readiness score bands),
    ...
    for s in top:
        ...
        if not is_scored:
            ...
            if entry_eligible:
                council_note = f"PRIME | Readiness {readiness:.1f} | {conf_count}/10 confirmations | ENTRY READY"
            else:
                gap = max(0, ENTRY_MIN_CONFIRMATIONS - conf_count)
                if gap > 0:
                    council_note = f"PRIME | Readiness {readiness:.1f} | {conf_count}/10 conf | needs {gap} more to enter"
                else:
                    council_note = f"PRIME | Readiness {readiness:.1f} | {conf_count}/10 conf | above EMA needed"
        elif tier == "NOW":
            council_note = f"BUILDING | Readiness {readiness:.1f} | {conf_count}/10 conf | non-trading — needs PRIME tier"
        elif tier == "WATCH":
            council_note = f"WATCHING | Readiness {readiness:.1f} | {conf_count}/10 conf"
        else:
            council_note = f"TRACKING | Readiness {readiness:.1f} | {conf_count}/10 conf"
        ...
        prices[symbol] = {
            ...
            "targets": {
                ...
                "profit_25": profit_25,
                "profit_50": profit_50,
                "signal": DISPLAY_TIER.get(tier, tier),
                "backend_tier": tier,
    ...
    for symbol, pdata in prices.items():
        pdata.setdefault("display_tier", DISPLAY_TIER.get(pdata.get("signal_tier", "TRACKING"), "MONITOR"))
...
def write_outputs(payload: Dict):
    ...
    live_data["watchlist"] = list(live_data["prices"].values())
