...
def _why_on_watchlist(signal_data, watchlist_data):
    ...
    if entry_eligible:
        if tier == "STRONG_NOW":
            return f"Entry-ready at STRONG_NOW / PRIME tier (readiness {readiness:.0f}, {conf_count}/10 confirmations). Highest conviction — bot will buy with 2.0x sizing."
        else:
            # Should not happen under Option B (only STRONG_NOW is tradeable)
            return f"Entry flag set in {tier} tier (readiness {readiness:.0f}, {conf_count}/10 confirmations) — review gate logic."
    elif tier == "STRONG_NOW":
        gap_conf = max(0, 5 - conf_count)
        if gap_conf > 0:
            return f"STRONG_NOW / PRIME tier (readiness {readiness:.0f}) but missing {gap_conf} confirmation(s) to reach entry gate ({conf_count}/10, need 5+)."
        elif not watchlist_data.get("above_ema", signal_data.get("above_ema20", False)):
            return f"STRONG_NOW / PRIME tier (readiness {readiness:.0f}, {conf_count}/10 confirmations) but price is not above EMA."
        else:
            return f"STRONG_NOW / PRIME tier (readiness {readiness:.0f}, {conf_count}/10 confirmations). Entry conditions met if capital is available."
    elif tier == "NOW":
        gap = 78 - readiness
        if gap > 0:
            return f"NOW / BUILDING tier (readiness {readiness:.0f}). Non-trading — needs {gap:.0f} more readiness points to reach STRONG_NOW / PRIME entry tier."
        else:
            return f"NOW / BUILDING tier at the threshold. Needs STRONG_NOW confirmation and 5/10 confirmations with price above EMA to become tradeable."
    elif tier == "MONITOR":
...
def _what_triggers_buy(signal_data, watchlist_data):
    ...
    missing = []
    if tier not in ("STRONG_NOW",):
        gap = 78 - readiness
        if gap > 0:
            missing.append(f"reach STRONG_NOW / PRIME tier at readiness 78 (currently {readiness:.0f})")
        else:
            missing.append("cross into STRONG_NOW / PRIME tier (readiness already near 78)")
    if conf_count < 5:
        missing.append(f"reach 5+ confirmations (currently {conf_count})")
    ...
    if not missing:
        return "STRONG_NOW / PRIME conditions met — entry will trigger when capital is available."

    return "To become tradeable: " + "; ".join(missing) + "."
