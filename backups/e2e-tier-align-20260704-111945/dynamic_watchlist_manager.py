...
def assign_tier(readiness_score: float, entry_eligible: bool = False) -> str:
    """Assign action-based tier from readiness score and entry eligibility."""
    if entry_eligible:
        return "PRIME"
    if readiness_score >= TIER_BUILDING_MIN:
        return "BUILDING"
    return "TRACKING"
...
def detect_tier_changes(
    ...
    for sig in current_symbols:
        ...
        if old_tier and old_tier != new_tier:
            change_type = "promote" if (old_tier == "TRACKING" and new_tier in ("BUILDING", "PRIME")) or (old_tier == "BUILDING" and new_tier == "PRIME") else "demote"
            changes.append({
                "symbol": symbol,
...
def build_watchlist(signals: List[Dict]) -> Dict:
    ...
    # PRIME = entry_eligible, BUILDING = not eligible but >= 55, TRACKING = < 55
    DISPLAY_TIER = {
        "PRIME": "PRIME",
        "BUILDING": "BUILDING",
        "TRACKING": "TRACKING",
    ...
    tiers = {"PRIME": [], "BUILDING": [], "TRACKING": []}
    for s in top:
        readiness = s.get("readiness_score", 0)
        total_score = s.get("total_score", 0)
        is_scored = total_score > 0 or readiness > 0
        tier = assign_tier(readiness, s.get("entry_eligible", False)) if is_scored else "TRACKING"
    ...
    for s in top:
        ...
        tier = assign_tier(readiness, entry_eligible)
        ...
        if not is_scored:
            ...
            council_note = f"TRACKING | Readiness {readiness:.1f} | {conf_count}/10 conf | below building threshold"
    ...
    for symbol, pdata in prices.items():
        ...
        if symbol in high_beta_symbols and high_beta_pct >= MAX_HIGH_BETA_DEPLOYED_PCT:
            ...
            reason = f"Diversification candidate (readiness {pdata.get('readiness_score', 0):.1f}, {pdata.get('confirmation_count', 0)}/10 conf)"
        elif held_info:
